...
/// derived deterministically from the channel name, with the channel number centred and the title along the
/// bottom (abbreviated to initials, then ellipsised, when it is too long to fit). Rendered once per
/// (number, name) with ffmpeg and cached in memory.
/// </summary>
public class DefaultLogoService
{
    ...
    /// </summary>
    /// <param name="number">The channel number, drawn in the centre.</param>
    /// <param name="name">The channel name, hashed to pick the background colour.</param>
    /// <param name="cancellationToken">A cancellation token.</param>
    /// <returns>The PNG bytes, or <c>null</c>.</returns>
    public async Task<byte[]?> GetAsync(int number, string name, CancellationToken cancellationToken)
    {
        var key = number.ToString(CultureInfo.InvariantCulture) + "|" + (name ?? string.Empty);
        if (_cache.TryGetValue(key, out var cached))
        {
            return cached;
        }

        var bytes = await GenerateAsync(number, name ?? string.Empty, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<byte[]?> GenerateAsync(int number, string name, CancellationToken cancellationToken)
    {
        ...
        if (font is not null)
        {
            var safeFont = font.Replace("\\", "/", StringComparison.Ordinal);
            var fc = "0x" + text;

            // Number, large and vertically centred.
            var numberSize = (Size * 2 / 5).ToString(CultureInfo.InvariantCulture);
            var vf = "drawtext=fontfile='" + safeFont + "':text='" + label
                + "':fontcolor=" + fc + ":fontsize=" + numberSize + ":x=(w-tw)/2:y=(h-th)/2";

            // Title along the bottom, on one or two lines (or dropped) so it never overflows.
            ...
            for (var i = 0; i < lines.Length; i++)
            {
                ...
                vf += ",drawtext=fontfile='" + safeFont + "':text='" + SanitizeForDrawtext(lines[i])
    ...
    // Maps the channel name to a stable pastel background and a darker same-hue number, matching the
